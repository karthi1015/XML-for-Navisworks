...
from xml_creator import get_revit_description, get_archicad_description, write_to_xml_archicad, write_to_xml_revit
...
class External(QThread):
    ...
    def run(self):
        ...
        if len(filename) > 0:  
            ...
            if len(get_revit_description(filename=filename)) > 0:
                self.countChanged.emit(1)
                write_to_xml_revit(file_name=str(filename))
              
            if len(get_archicad_description(filename))  > 0: 
                self.countChanged.emit(1)
                write_to_xml_archicad(file_name=str(filename)) 
...
class Actions(QtGui.QMainWindow):
    ...
    def initUI(self):
        ...
        self.button = QtGui.QPushButton('Selecteer een .IFC bestand', self)
    ...
    def onCountChanged(self, value):
        ...
        if value == 0:
            ...
            self.label.setText('...IFC data wordt verwerkt en omgezet naar XML.')
        
        if value == 1: 
            self.progress.setRange(0,1)
            self.label.setText('...Het XML bestand is gereed.')
